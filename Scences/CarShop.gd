...
onready var coins_lable = $total_coins/total_coins_lbl
...
func update_coins_lable():
	coins_lable.text = str(get_total_coins())
...
func _ready():
	...
	screen_car.position.y = 355
	screen_car.scale.x = 1.5
	screen_car.scale.y = 1.5
	if current_car > 1:
